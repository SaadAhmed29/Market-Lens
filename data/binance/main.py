...
def run() -> None:
    """Main entry point: run automated data pipeline for Binance."""
    logger.info("Starting Binance data pipeline...")
    
    # Load configs from DB
    configs = load_data_config("binance")
    ...
    for row in configs:
        ...
        logger.info(f"Executing Binance download plan for {symbol} ({time_horizon})...")
        fetcher = DataFetcher(fetcher_config)
        
        try:
            fetcher.download(symbol)
            update_data_config_dates("binance", symbol, time_horizon, start_date, now_dt)
