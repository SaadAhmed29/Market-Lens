...
export default function StrategyBuilderDetailPage() {
    ...
    if (isLoading) {
        return (
            <PageWrapper title={`STRAT_BUILDER_${requestId?.substring(0, 8) || 'LOADING'}`}>
    ...
    if (isError || !data || !data.request) {
        return (
            <PageWrapper title="STRATEGY_BUILDER_ERROR">
                <EmptyState message="FAILED_TO_LOAD_DETAIL" />
            </PageWrapper>
        )
    }
