...
def generate_plots(returns: pd.Series, output_dir: str):
    ...
    try:
        ...
        qs.plots.rolling_sharpe(returns, savefig=os.path.join(output_dir, 'rolling_sharpe.png'), show=False)
        qs.plots.rolling_volatility(returns, savefig=os.path.join(output_dir, 'rolling_volatility.png'), show=False)
